...
output=$(run_hook "$HOOKS_DIR/legacy/branch-guard.sh" "$BG_FIXTURE_MAIN_DENY")
...
output=$(run_hook "$HOOKS_DIR/legacy/branch-guard.sh" "$BG_FIXTURE_FEATURE_ALLOW")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/branch-guard.sh" "$BG_FIXTURE_MAIN_NONSOURCE")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/branch-guard.sh" "$BG_FIXTURE_PLAN")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/doc-gate.sh" "$DOC_FIXTURE_NO_HEADER")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" == "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/doc-gate.sh" "$DOC_FIXTURE_WITH_HEADER")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/doc-gate.sh" "$DOC_FIXTURE_NO_DECISION")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" == "deny" ]]; then
...
output=$(run_hook "$HOOKS_DIR/legacy/doc-gate.sh" "$DOC_FIXTURE_WITH_DECISION")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(CLAUDE_PROJECT_DIR="$TG_TEST_DIR" run_hook "$HOOKS_DIR/legacy/test-gate.sh" "$TG_FIXTURE_COLD")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
{"tool_name":"Write","tool_input":{"file_path":"$TG_TEST_DIR/src/main.ts","content":"console.log('test');\n"}}
EOF
output=$(CLAUDE_PROJECT_DIR="$TG_TEST_DIR" run_hook "$HOOKS_DIR/legacy/test-gate.sh" "$TG_FIXTURE_PASS")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
...
output=$(CLAUDE_PROJECT_DIR="$TG_TEST_DIR" run_hook "$HOOKS_DIR/legacy/test-gate.sh" "$TG_FIXTURE_SRC")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
...
output=$(run_hook "$HOOKS_DIR/legacy/mock-gate.sh" "$MG_FIXTURE_NONTEST")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
...
output=$(CLAUDE_PROJECT_DIR="$MG_TEST_DIR" run_hook "$HOOKS_DIR/legacy/mock-gate.sh" "$MG_FIXTURE_INTERNAL_MOCK")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
context=$(echo "$output" | jq -r '.hookSpecificOutput.additionalContext // empty' 2>/dev/null)
...
# --- Test: worktree path exclusion for defprog-gate and doc-gate ---
echo "--- worktree path exclusion tests ---"

# Setup: temp git repo on feature branch with MASTER_PLAN.md (bypass gates 1-4)
WT_TEST_DIR=$(mktemp -d)
git init "$WT_TEST_DIR" >/dev/null 2>&1
(cd "$WT_TEST_DIR" && git checkout -b feature/worktree-test >/dev/null 2>&1 && \
    mkdir -p .claude/.worktrees/test-wt/hooks .claude && \
    cat > MASTER_PLAN.md <<'PLANEOF'
## Identity
Test project for worktree path exclusion testing
...
# Test: defprog-gate skips hooks file at worktree path (except:pass should be ignored)
WT_FIXTURE_DEFPROG="$FIXTURES_DIR/worktree-defprog.json"
cat > "$WT_FIXTURE_DEFPROG" <<EOF
{"tool_name":"Write","tool_input":{"file_path":"$WT_TEST_DIR/.claude/.worktrees/test-wt/hooks/source-lib.sh","content":"# Source library for hooks\ntry:\n    update_widget()\nexcept Exception:\n    pass\n"}}
EOF

output=$(CLAUDE_PROJECT_DIR="$WT_TEST_DIR" run_hook "$HOOKS_DIR/pre-write.sh" "$WT_FIXTURE_DEFPROG")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
context=$(echo "$output" | jq -r '.hookSpecificOutput.additionalContext // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]] && ! echo "$context" | grep -q "silently swallows" 2>/dev/null; then
    pass "defprog-gate — skip hooks file at worktree path"
else
    fail "defprog-gate — skip hooks file at worktree path" "should skip but got decision=$decision context=$context"
fi
rm -f "$WT_FIXTURE_DEFPROG"

# Test: doc-gate skips hooks file at worktree path (no header deny)
WT_FIXTURE_DOC="$FIXTURES_DIR/worktree-doc.json"
cat > "$WT_FIXTURE_DOC" <<EOF
{"tool_name":"Write","tool_input":{"file_path":"$WT_TEST_DIR/.claude/.worktrees/test-wt/hooks/new-hook.sh","content":"set -euo pipefail\necho hello\n"}}
EOF

output=$(CLAUDE_PROJECT_DIR="$WT_TEST_DIR" run_hook "$HOOKS_DIR/pre-write.sh" "$WT_FIXTURE_DOC")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
    pass "doc-gate — skip hooks file at worktree path"
else
    fail "doc-gate — skip hooks file at worktree path" "should skip but got deny: $output"
fi
rm -f "$WT_FIXTURE_DOC"

# Test: legacy doc-gate.sh also skips worktree hooks path
WT_FIXTURE_LEGACY="$FIXTURES_DIR/worktree-legacy-doc.json"
cat > "$WT_FIXTURE_LEGACY" <<EOF
{"tool_name":"Write","tool_input":{"file_path":"$WT_TEST_DIR/.claude/.worktrees/test-wt/hooks/new-hook.sh","content":"set -euo pipefail\necho hello\n"}}
EOF

output=$(run_hook "$HOOKS_DIR/legacy/doc-gate.sh" "$WT_FIXTURE_LEGACY")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
    pass "legacy doc-gate.sh — skip hooks file at worktree path"
else
    fail "legacy doc-gate.sh — skip hooks file at worktree path" "should skip but got deny: $output"
fi
rm -f "$WT_FIXTURE_LEGACY"

# Test: main .claude/hooks/ path still excluded (regression check)
WT_FIXTURE_MAIN="$FIXTURES_DIR/worktree-main-hooks.json"
cat > "$WT_FIXTURE_MAIN" <<EOF
{"tool_name":"Write","tool_input":{"file_path":"$WT_TEST_DIR/.claude/hooks/source-lib.sh","content":"set -euo pipefail\necho hello\n"}}
EOF

output=$(CLAUDE_PROJECT_DIR="$WT_TEST_DIR" run_hook "$HOOKS_DIR/pre-write.sh" "$WT_FIXTURE_MAIN")
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" != "deny" ]]; then
    pass "defprog/doc-gate — still skip main .claude/hooks/ path (regression)"
else
    fail "defprog/doc-gate — still skip main .claude/hooks/ path (regression)" "should skip but got deny: $output"
fi
rm -f "$WT_FIXTURE_MAIN"

safe_cleanup "$WT_TEST_DIR" "$SCRIPT_DIR"
echo ""
...
if [[ -f "$FIXTURES_DIR/auto-review-safe.json" ]]; then
    output=$(run_hook "$HOOKS_DIR/legacy/auto-review.sh" "$FIXTURES_DIR/auto-review-safe.json")
    if echo "$output" | jq -e '.hookSpecificOutput.permissionDecision' > /dev/null 2>&1; then
        decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision')
...
auto_review_assert_approved() {
    ...
    if [[ -f "$FIXTURES_DIR/$fixture" ]]; then
        local output decision
        output=$(run_hook "$HOOKS_DIR/legacy/auto-review.sh" "$FIXTURES_DIR/$fixture")
        decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
        if [[ "$decision" == "allow" ]]; then
...
if [[ -f "$FIXTURES_DIR/plan-validate-non-plan.json" ]]; then
    output=$(run_hook "$HOOKS_DIR/legacy/plan-validate.sh" "$FIXTURES_DIR/plan-validate-non-plan.json")
...
output=$(echo "$PLAN_CHECK_INPUT" | CLAUDE_PROJECT_DIR="$PC_TEST_DIR" bash "$HOOKS_DIR/legacy/plan-check.sh" 2>/dev/null) || true
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [[ "$decision" == "deny" ]]; then
    reason=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecisionReason // empty' 2>/dev/null)
