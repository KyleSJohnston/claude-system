...
write_statusline_cache() {
    ...
    jq -n \
        ...
        '{dirty:($dirty|tonumber),worktrees:($wt|tonumber),updated:($ts|tonumber),agents_active:($sa_count|tonumber),agents_types:$sa_types,agents_total:($sa_total|tonumber),todo_project:($todo_project|tonumber),todo_global:($todo_global|tonumber),lifetime_cost:($lifetime_cost|tonumber),initiative:$initiative,phase:$phase,active_initiatives:($active_initiatives|tonumber),total_phases:($total_phases|tonumber)}' \
